...
* Every file prefixed with `jquery-ui` has to be overriden carefully. If you create an new theme for an existing projet, check if some jQuery UI widgets you need to keep are not already in use within standard or custom features. If you need to keep some jQuery UI features, chances are you'll also have to copy/paste/adapt some styles to your new theme because nothing is included.
